  ...
  const data = await withRLS(user.userId, user.role, user.claims, async (client) => {
    const result = await client.query(
      `SELECT DISTINCT ON (conversation_id)
        m.conversation_id, m.content as last_message, m.created_at as last_message_at,
       ...
       LEFT JOIN user_profiles rup ON m.receiver_id = rup.id
...
router.post('/messages', async (event) => {
  ...
  const data = await withRLS(user.userId, user.role, user.claims, async (client) => {
    const convId = body.conversation_id ||
      [user.userId, body.receiver_id].sort().join('-');

    const result = await client.query(
      `INSERT INTO messages (sender_id, receiver_id, content, conversation_id, message_type)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [user.userId, body.receiver_id, body.content, convId, body.message_type || 'text']
    );
