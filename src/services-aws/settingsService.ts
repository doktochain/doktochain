...
export const settingsService = {
  async getUserSettings(userId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<UserSettings>('/user-settings', {
        params: { user_id: userId, single: true },
      });
  ...
  async updateUserSettings(
    ...
    try {
      const { data, error } = await api.post<UserSettings>('/user-settings', {
        ...updates,
        user_id: userId,
  ...
  async getPrivacySettings(userId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<PrivacySettings>('/user-privacy-settings', {
        params: { user_id: userId, single: true },
      });
  ...
  async updatePrivacySettings(
    ...
    try {
      const { data, error } = await api.post<PrivacySettings>('/user-privacy-settings', {
        ...updates,
        user_id: userId,
        updated_at: new Date().toISOString(),
        _upsert: true,
  ...
  async getAccessibilitySettings(userId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<AccessibilitySettings>('/user-accessibility-settings', {
        params: { user_id: userId, single: true },
      });
  ...
  async updateAccessibilitySettings(
    ...
    try {
      const { data, error } = await api.post<AccessibilitySettings>('/user-accessibility-settings', {
        ...updates,
        user_id: userId,
        updated_at: new Date().toISOString(),
        _upsert: true,
  ...
  async getLoginHistory(userId: string, limit = 20): Promise<{
    ...
    try {
      const { data, error } = await api.get<LoginHistory[]>('/login-history', {
        params: { user_id: userId, order: 'login_timestamp.desc', limit },
      });
  ...
  async getTrustedDevices(userId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<TrustedDevice[]>('/trusted-devices', {
        params: { user_id: userId, is_active: true, order: 'last_used.desc' },
      });
  ...
  async removeTrustedDevice(deviceId: string): Promise<{
    ...
    try {
      const { error } = await api.put(`/trusted-devices/${deviceId}`, {
        is_active: false,
      });
  ...
  async requestDataExport(userId: string, type: string): Promise<{
    ...
    try {
      const { data, error } = await api.post<DataExportRequest>('/data-export-requests', {
        user_id: userId,
        request_type: type,
        status: 'pending',
      });
  ...
  async getDataExportRequests(userId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<DataExportRequest[]>('/data-export-requests', {
        params: { user_id: userId, order: 'requested_at.desc' },
      });
  ...
  async requestAccountDeletion(userId: string, reason?: string): Promise<{
    ...
    try {
      ...
      const { data, error } = await api.post<any>('/account-deletion-requests', {
        user_id: userId,
        reason,
        status: 'pending',
  ...
  async initializeSettings(userId: string): Promise<{
    ...
    try {
      await Promise.all([
        api.post('/user-settings', { user_id: userId, _upsert: true }),
        api.post('/user-privacy-settings', { user_id: userId, _upsert: true }),
        api.post('/user-accessibility-settings', { user_id: userId, _upsert: true }),
      ]);
