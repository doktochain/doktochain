...
export const hrmReportsService = {
  async getDashboardStats(): Promise<HRMDashboardStats> {
    ...
    const { data: totalEmployees } = await api.get<any[]>('/user-profiles', {
      params: { role: ['staff', 'provider', 'admin'], count_only: true },
    });

    const { data: todayAttendance } = await api.get<any[]>('/staff-attendance', {
      params: { date: today },
    });
    ...
    const { data: newHires } = await api.get<any[]>('/user-profiles', {
      params: { join_date_from: firstOfMonth, count_only: true },
    });

    const { data: monthAttendance } = await api.get<any[]>('/staff-attendance', {
      params: { start_date: firstOfMonth, end_date: today },
    });
  ...
  async getAttendanceReport(
    ...
    const { data: employees, error } = await api.get<any[]>('/user-profiles', { params });
  ...
  async getLeaveReport(year?: number): Promise<LeaveReport[]> {
    ...
    const { data: employees, error: empError } = await api.get<any[]>('/user-profiles', {
      params: { role: ['staff', 'provider', 'admin'] },
    });

    if (empError) throw empError;

    const { data: leaveTypes, error: ltError } = await api.get<any[]>('/leave-types', {
  ...
  async getPayrollReport(month: number, year: number): Promise<PayrollReport[]> {
    ...
    const { data: employees, error } = await api.get<any[]>('/user-profiles', {
      params: { ids: payslips.map(p => p.user_id) },
    });
  ...
  async getHeadcountReport() {
    const { data: employees, error } = await api.get<any[]>('/user-profiles', {
      params: { role: ['staff', 'provider', 'admin'], include: 'department,designation' },
    });
  ...
  async getTurnoverReport(months: number = 12) {
    ...
    const { data: newHires, error: hiresError } = await api.get<any[]>('/user-profiles', {
      params: {
        join_date_from: startDate.toISOString().split('T')[0],
        role: ['staff', 'provider', 'admin'],
      },
    });
