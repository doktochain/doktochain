...
export const platformSettingsService = {
  async getWebsiteSettings(): Promise<WebsiteSettings | null> {
    try {
      const { data, error } = await api.get<WebsiteSettings>('/platform-website-settings', {
        params: { limit: 1, single: true },
      });
  ...
  async updateWebsiteSettings(settings: Partial<WebsiteSettings>): Promise<WebsiteSettings | null> {
    try {
      ...
      const { data, error } = await api.put<WebsiteSettings>(`/platform-website-settings/${current.id}`, {
        ...settings,
  ...
  async getClinicSettings(): Promise<ClinicSettings | null> {
    try {
      const { data, error } = await api.get<ClinicSettings>('/platform-clinic-settings', {
        params: { limit: 1, single: true },
      });
  ...
  async updateClinicSettings(settings: Partial<ClinicSettings>): Promise<ClinicSettings | null> {
    try {
      ...
      const { data, error } = await api.put<ClinicSettings>(`/platform-clinic-settings/${current.id}`, {
        ...settings,
        updated_at: new Date().toISOString(),
      });
  ...
  async getAppSettings(): Promise<AppSettings | null> {
    try {
      const { data, error } = await api.get<AppSettings>('/platform-app-settings', {
        params: { limit: 1, single: true },
      });
  ...
  async updateAppSettings(settings: Partial<AppSettings>): Promise<AppSettings | null> {
    try {
      ...
      const { data, error } = await api.put<AppSettings>(`/platform-app-settings/${current.id}`, {
        ...settings,
        updated_at: new Date().toISOString(),
      });
  ...
  async getSystemSettings(): Promise<SystemSettings | null> {
    try {
      const { data, error } = await api.get<SystemSettings>('/platform-system-settings', {
        params: { limit: 1, single: true },
      });
  ...
  async updateSystemSettings(settings: Partial<SystemSettings>): Promise<SystemSettings | null> {
    try {
      ...
      const { data, error } = await api.put<SystemSettings>(`/platform-system-settings/${current.id}`, {
        ...settings,
        updated_at: new Date().toISOString(),
      });
  ...
  async getFinanceSettings(): Promise<FinanceSettings | null> {
    try {
      const { data, error } = await api.get<FinanceSettings>('/platform-finance-settings', {
        params: { limit: 1, single: true },
      });
  ...
  async updateFinanceSettings(settings: Partial<FinanceSettings>): Promise<FinanceSettings | null> {
    try {
      ...
      const { data, error } = await api.put<FinanceSettings>(`/platform-finance-settings/${current.id}`, {
        ...settings,
        updated_at: new Date().toISOString(),
      });
  ...
  async getAccountSettings(): Promise<AccountSetting[]> {
    try {
      const { data, error } = await api.get<AccountSetting[]>('/platform-account-settings', {
        params: { order: 'category,display_order' },
      });
  ...
  async updateAccountSetting(key: string, value: any): Promise<AccountSetting | null> {
    try {
      const { data, error } = await api.put<AccountSetting>('/platform-account-settings', {
        value,
        updated_at: new Date().toISOString(),
        _filter: { key },
      });
  ...
  async getOtherSettings(category: string): Promise<any> {
    try {
      const { data, error } = await api.get<any>('/platform-other-settings', {
        params: { category, single: true },
      });
  ...
  async updateOtherSettings(category: string, settingsData: any, description?: string): Promise<any> {
    try {
      const { data, error } = await api.post<any>('/platform-other-settings', {
        category,
  ...
  async getAuditLog(limit: number = 100): Promise<any[]> {
    try {
      const { data, error } = await api.get<any[]>('/settings-audit-log', {
        params: { order: 'changed_at.desc', limit },
      });
