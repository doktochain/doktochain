...
export const pharmacyMarketplaceService = {
  async searchPharmacies(filters: PharmacySearchFilters = {}): Promise<{
    ...
    try {
      ...
      if (filters.has_delivery) {
        params.accepts_delivery = true;
      }

      if (filters.min_rating) {
        params.rating_average_gte = filters.min_rating;
      }

      if (filters.postal_code) {
        params.postal_code_prefix = filters.postal_code;
      }
  ...
  async getPatientOrders(patientId: string): Promise<{
    ...
    try {
      const { data, error } = await api.get<PharmacyOrder[]>('/pharmacy-orders', {
        params: {
  ...
  async updateOrderStatus(
    ...
    try {
      const { data, error } = await api.put<PharmacyOrder>(`/pharmacy-orders/${orderId}`, {
        status,
  ...
  async createOrderWithItems(orderData: {
    ...
    try {
      ...
      const { data: order, error: orderError } = await api.post<PharmacyOrder>('/pharmacy-orders', {
        order_number: orderNumber,
        patient_id: orderData.patient_id,
        pharmacy_id: orderData.pharmacy_id,
        order_type: 'otc',
      ...
      const { error: itemsError } = await api.post('/order-items', orderItems);
