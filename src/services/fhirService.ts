...
export const fhirService = {
  ...
  async syncFHIRData(patientId: string): Promise<HealthSyncStatus> {
    try {
      const [obsResult, condResult, immResult] = await Promise.all([
        supabase.from('fhir_observations').select('id', { count: 'exact', head: true }).eq('patient_id', patientId),
        supabase.from('fhir_conditions').select('id', { count: 'exact', head: true }).eq('patient_id', patientId),
        supabase.from('immunizations').select('id', { count: 'exact', head: true }).eq('patient_id', patientId),
      ]);

      const totalRecords = (obsResult.count || 0) + (condResult.count || 0) + (immResult.count || 0);
  ...
  async getSyncStatus(patientId: string): Promise<HealthSyncStatus> {
    const [obsResult, condResult, immResult] = await Promise.all([
      supabase.from('fhir_observations').select('effective_datetime').eq('patient_id', patientId).order('effective_datetime', { ascending: false }).limit(1).maybeSingle(),
      supabase.from('fhir_conditions').select('recorded_date').eq('patient_id', patientId).order('recorded_date', { ascending: false }).limit(1).maybeSingle(),
      supabase.from('immunizations').select('created_at').eq('patient_id', patientId).order('created_at', { ascending: false }).limit(1).maybeSingle(),
    ]);
    ...
    const totalRecords = (obsCount.count || 0) + (condCount.count || 0) + (immCount.count || 0);
