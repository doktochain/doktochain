...
export const clinicalNotesService = {
  ...
  async getNote(noteId: string, providerId?: string): Promise<ClinicalNote | null> {
    const { data, error } = await supabase
      .from('clinical_notes')
      .select('*, appointments(*), patients(*, user_profiles(*)), providers(*, user_profiles(*))')
      .eq('id', noteId)
      .maybeSingle();
  ...
  async getAppointmentNote(appointmentId: string, providerId?: string): Promise<ClinicalNote | null> {
    ...
    if (data && providerId) {
      try {
        await auditLog.dataAccessed('clinical_note', data.id, providerId, 'provider', {
          action: 'read', appointment_id: appointmentId, patient_id: data.patient_id,
        });
      } catch {}
    }
  ...
  async getProviderNotes(providerId: string, limit: number = 50): Promise<ClinicalNote[]> {
    const { data, error } = await supabase
      .from('clinical_notes')
      .select('*, patients(*, user_profiles(first_name, last_name)), appointments(*)')
  ...
  async getTemplates(specialty?: string): Promise<NoteTemplate[]> {
    let query = supabase
      ...
      .select('id, name, specialty, template_type, template_content, is_default')
      .eq('is_active', true)
