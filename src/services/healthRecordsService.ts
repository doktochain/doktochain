...
class HealthRecordsService {
  async getLabResults(patientId: string, filters?: { category?: string; startDate?: string; endDate?: string; providerId?: string }) {
    ...
    if (filters?.category) {
      query = query.eq('test_category', filters.category);
    }
    if (filters?.startDate) {
      query = query.gte('result_date', filters.startDate);
    }
    if (filters?.endDate) {
      query = query.lte('result_date', filters.endDate);
    }
  ...
  async getSyncStatus(patientId: string) {
    return await supabase
      .from('health_record_sync_status')
      .select('*')
      .eq('patient_id', patientId)
  ...
  async getHealthTimeline(patientId: string, startDate?: string, endDate?: string): Promise<HealthTimelineEvent[]> {
    ...
    if (allergyRes.data) {
      allergyRes.data.forEach((allergy) => {
        events.push({
          id: allergy.id,
  ...
  async addLabResult(data: Omit<LabResult, 'id' | 'created_at' | 'updated_at'>) {
    ...
    const result = await supabase.from('lab_results').insert(data).select().single();
    if (result.data) {
      try {
  ...
  async addMedicationHistory(data: Omit<MedicationHistory, 'id' | 'created_at' | 'updated_at'>) {
    ...
    const result = await supabase.from('medication_history').insert(data).select().single();
    if (result.data) {
      try {
        await blockchainAuditService.logEvent({
          eventType: 'record_accessed',
  ...
  async addAllergy(data: Omit<Allergy, 'id' | 'created_at' | 'updated_at'>) {
    ...
    const result = await supabase.from('allergies').insert(data).select().single();
    if (result.data) {
      try {
        await blockchainAuditService.logEvent({
          eventType: 'record_accessed',
  ...
  async addImmunization(data: Omit<Immunization, 'id' | 'created_at' | 'updated_at'>) {
    const result = await supabase.from('immunizations').insert(data).select().single();
    if (result.data) {
      try {
        await blockchainAuditService.logEvent({
          eventType: 'record_accessed',
  ...
  async updateLabResult(id: string, data: Partial<LabResult>, providerId?: string) {
    if (providerId) {
      const { data: existing } = await supabase.from('lab_results').select('patient_id').eq('id', id).maybeSingle();
      if (existing) {
  ...
  async updateMedication(id: string, data: Partial<MedicationHistory>, providerId?: string) {
    if (providerId) {
      const { data: existing } = await supabase.from('medication_history').select('patient_id').eq('id', id).maybeSingle();
      if (existing) {
  ...
  async updateAllergy(id: string, data: Partial<Allergy>, providerId?: string) {
    if (providerId) {
      const { data: existing } = await supabase.from('allergies').select('patient_id').eq('id', id).maybeSingle();
      if (existing) {
  ...
  async createRecordShare(data: Omit<RecordShare, 'id' | 'created_at' | 'updated_at'>) {
    const result = await supabase.from('record_shares').insert(data).select().single();
    if (result.data) {
      try {
        await blockchainAuditService.logEvent({
