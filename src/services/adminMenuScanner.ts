/**
 * Admin Menu Scanner
 * Dynamically extracts and parses the admin menu structure from Sidebar
 * Automatically discovers new menus and submenus
 */

export interface MenuResource {
  id: string;                    // Unique identifier (e.g., "admin_dashboard_analytics")
  label: string;                 // Display name (e.g., "Dashboard - Analytics")
  path: string;                  // URL path (e.g., "/dashboard/admin/dashboard/analytics")
  section: string;               // Section category (e.g., "Main Menu", "Clinic Management")
  parentMenu: string | null;     // Parent menu label (null for top-level items)
  hasSubmenu: boolean;           // Whether this item has submenus
  icon?: string;                 // Icon name if available
}
...
/**
 * Generate a unique resource ID from a path
 */
...
/**
 * Scan and extract all admin menu resources
 */
export function scanAdminMenuResources(): MenuResource[] {
  ...
  adminMenuStructure.forEach((section) => {
    section.items.forEach((item) => {
      ...
      if (item.submenu) {
        // Add parent menu item (if it has a dedicated page)
        if (item.href) {
          resources.push({
...
/**
 * Check if a new menu has been added (not in permissions yet)
 */
