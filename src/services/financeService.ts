...
export class FinanceService {
  ...
  static async getExpenses(
    ...
    return { data: data as PlatformExpense[], count: count || 0 };
  }
  ...
  static async getExpenseSummary(startDate?: string, endDate?: string) {
    let query = supabase
      .from('platform_expenses')
      .select('amount, status, expense_category');
  ...
  static async getIncome(
    ...
    if (filters?.source) {
      query = query.eq('income_source', filters.source);
    }
    if (filters?.startDate) {
    ...
    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as PlatformIncome[], count: count || 0 };
  }
  ...
  static async getInvoices(
    ...
    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as PlatformInvoice[], count: count || 0 };
  }
  ...
  static async getRefundRequests(
    ...
    if (filters?.type) {
      query = query.eq('refund_type', filters.type);
    }

    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as RefundRequest[], count: count || 0 };
  }
  ...
  // Marketplace Transaction Logs Methods
  static async getTransactionLogs(
    ...
    let query = supabase
      .from('marketplace_transaction_logs')
      .select('*', { count: 'exact' })
      .order('created_at', { ascending: false })
      .range(offset, offset + limit - 1);
    ...
    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as MarketplaceTransactionLog[], count: count || 0 };
  }
  ...
  // Delivery Cost Analytics Methods
  static async getDeliveryAnalytics(
    ...
    if (filters?.provider) {
      query = query.eq('delivery_provider', filters.provider);
    }
    if (filters?.startDate) {
    ...
    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as DeliveryCostAnalytics[], count: count || 0 };
  }
  ...
  static async createOrUpdateDeliveryAnalytics(analytics: Partial<DeliveryCostAnalytics>) {
    const { data, error } = await supabase
      .from('delivery_cost_analytics')
      .upsert([analytics], {
        onConflict: 'period_date,pharmacy_id,delivery_provider,region',
      })
      .select()
      .single();
  ...
  static async getDeliveryAnalyticsSummary(startDate?: string, endDate?: string) {
    let query = supabase
      .from('delivery_cost_analytics')
      .select('total_deliveries, total_cost, revenue_from_deliveries, delivery_provider');
  ...
  static async getSettlements(
    ...
    let query = supabase
      ...
      .select('*, provider:provider_id(id, user_id, user_profiles:user_id(full_name, email))', { count: 'exact' })
    ...
    const { data, error, count } = await query;
    if (error) throw error;
    return { data: data as ProviderSettlement[], count: count || 0 };
  }

  static async getProviderUnsettledBalances(): Promise<ProviderUnsettledBalance[]> {
    const { data: providers, error: provError } = await supabase
      .from('providers')
      .select('id, user_id, user_profiles:user_id(full_name, email)')
      .eq('is_active', true);
  ...
  static async createSettlement(settlement: {
    ...
    const { data, error } = await supabase
      ...
      .select('*, provider:provider_id(id, user_id, user_profiles:user_id(full_name, email))')
      .single();
