...
class CryptoAuditTrailService {
  ...
  async logEventSafe(params: {
    ...
    try {
      ...
      try {
        await supabase.from('audit_failures').insert({
  ...
  async getAuditTrail(filters?: {
    ...
    query = query.order('block_number', { ascending: false }).limit(filters?.limit || 100);
  ...
  async getPatientAccessLog(patientId: string, limit: number = 100): Promise<AuditTrailEntry[]> {
    const { data, error } = await supabase
      .from('blockchain_audit_log')
      .select('*')
      .or(`resource_id.eq.${patientId},action_data->>patient_id.eq.${patientId}`)
      .in('event_type', ['data_access', 'cross_provider_access', 'record_accessed', 'consent_granted', 'consent_revoked', 'consent_window_created'])
      .order('block_number', { ascending: false })
      .limit(limit);
  ...
  async verifyChainIntegrity(
    ...
    if (startBlock) {
      query = query.gte('block_number', startBlock);
    }
    if (endBlock) {
      query = query.lte('block_number', endBlock);
    }
  ...
  async runIntegrityCheck(
    ...
    const { data, error } = await supabase
      .from('blockchain_integrity_checks')
      .insert([{
