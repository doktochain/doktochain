...
interface ConversationWithProfile {
  ...
  created_at: string;
  patient: { id: string; first_name: string; last_name: string; email: string; profile_photo_url: string | null };
}
...
export default function ProviderInboxPage() {
  ...
  const loadConversations = async () => {
    if (!user) return;
    setLoading(true);
    const { data } = await messagingService.getConversations(user.id, 'provider');
  ...
  const filteredConversations = conversations.filter((c) => {
    ...
    if (filter === 'unread') return matchesSearch && c.unread_count_provider > 0;
    return matchesSearch;
  });

  const totalUnread = conversations.reduce((sum, c) => sum + (c.unread_count_provider || 0), 0);
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-9 h-9 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 text-sm font-bold shrink-0">
                          {conv.patient?.first_name?.[0] || 'P'}
                        </div>
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-900 dark:text-white text-sm truncate">{patientName}</p>
                    ...
                    {conv.priority === 'urgent' && (
                      <span className="inline-block mt-1 px-2 py-0.5 text-[10px] font-bold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded">
                        URGENT
                      </span>
                    )}
                  </button>
        ...
        {selectedConversation ? (
          <>
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center gap-3">
              ...
              <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-bold shrink-0">
                {selectedConversation.patient?.first_name?.[0] || 'P'}
              </div>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${
                        ...
                        {!isMe && (
                          <p className="text-xs font-medium mb-1 text-emerald-600 dark:text-emerald-400">
                            {getSenderName(msg)}
                          </p>
                        )}
                            ...
                            {msg.attachment_urls.map((url, i) => (
                              <a
                                ...
                                className={`flex items-center gap-2 text-xs px-2 py-1 rounded ${
                                  isMe ? 'bg-teal-700 text-teal-100 hover:bg-teal-800' : 'bg-gray-100 dark:bg-gray-700 text-teal-600 dark:text-teal-400 hover:bg-gray-200'
                                }`}
                              >
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Patient messages will appear in your inbox</p>
            </div>
          </div>
        )}
      </div>
